...
const unsigned char CrcHiTable [] = {
...
const unsigned char CrcLoTable [] = {
...
/*
*CRC16校验
*将数据的指针，需要校验的长度，和返回的crc的高低顺序传递进�?*/
int getCheckSum(const unsigned char* buffer, int length, int crc)
{
    ...
    if (crc == 0)
    {
            temp=(crcLo << 8 | crcHi);            //设置校验和按照低位在前，高位在后的顺�?            return temp;
    }
    else
    {       
          temp=(crcHi << 8 | crcLo);            //设置校验和按照高位在前，低位在后的顺�?          return temp;
    }
    return temp;
}
