...
{
	int i, day, day2;
	uint64_t sevenday[7] = {0};
	struct tm *utc;
	time_t current, utct;
	char daytemp[32] = {0};

	current=time(NULL);
	utct = mktime(gmtime(&current));
	day = utct / (24*3600);
	
	for (i=0;i<30;i++) {
		if (data.day[i].used) {
			day2 = mktime(gmtime(&(data.day[i].date))) / (24*3600);
			if (day >= day2 && day - day2 < 7) {
				sevenday[day - day2] = data.day[i].rx * 1024 + data.day[i].rxk;
			}
		}
	}
	printf("last 7 days flow:\n");
	for (i = 0; i < 7; i++)
	{
		printf("%s\n", getvalue(0, sevenday[i], 11, 1));
	}
	return;
}
