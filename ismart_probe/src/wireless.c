...
//	for_each_cmd(sectcmd) {
	for (sectcmd = &__start___cmd; sectcmd < &__stop___cmd;		\
	     sectcmd = (const struct cmd *)((char *)sectcmd + cmd_size))
    {
		if (sectcmd->parent)
			continue;
...
//		for_each_cmd(cmd) {
	    for (cmd = &__start___cmd; cmd < &__stop___cmd;		\
	         cmd = (const struct cmd *)((char *)cmd + cmd_size))
        {
			if (!cmd->handler)
				continue;
...
int wireless_set_channel(char *dev_name, int channel)
{
    int err = -1;
    int idx;
    ...
    char *cmd_2 = "channel";
    char cmd_3[4] = {0};
    char *argv[4] = {cmd_0, cmd_1, cmd_2, cmd_3};

    if (NULL == dev_name || channel < 0 || channel > 13)
    {
        return -1;
    }
    sprintf(cmd_3, "%d", channel);
    ...
    if ((idx = phy_lookup(dev_name)) >= 0)
    {
		idby = II_PHY_NAME;
		err = __handle_cmd(&nlstate, idby, sizeof(argv)/sizeof(argv[0]), argv, &cmd);
        printf("handle_cmd %d\n", err);
	}
