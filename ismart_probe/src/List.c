...
{
	if (NULL == plist || NULL == pData || 0 == nLen)
	{
		return NULL;
	}
...
PNode EnListTail(List *plist, void *pData, unsigned int nLen)
{
	if (NULL == plist || NULL == pData || 0 == nLen)
	{
		return NULL;
	...
	if(pnode != NULL)
	{
		...
		if (NULL != plist->front)
		{
			PNode ptailnode = plist->front;
			for(; NULL != ptailnode->next; ptailnode = ptailnode->next)
			{
				//使ptailnode指向list尾部
				;
			}
